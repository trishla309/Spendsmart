...
// ============================================================================
// 5. DYNAMIC PROGRAMMING KNAPSACK OPTIMIZATION
// Used for: Allocating finite pocket money against prioritized student needs.
// Explanation: The 0/1 Knapsack optimization determines the combination of items 
// that maximizes priority utility points without exceeding budget capacity.
// ============================================================================
...
export function knapsackOptimize(
  items: OptimizeItem[],
  capacity: number
): { selected: string[]; totalValue: number; remainingCapacity: number } {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    for (let w = 0; w <= capacity; w++) {
      if (item.cost <= w) {
        dp[i][w] = Math.max(
          dp[i - 1][w],
          dp[i - 1][w - item.cost] + item.priority
        );
      } else {
        dp[i][w] = dp[i - 1][w];
      }
  ...
  for (let i = n; i > 0; i--) {
    if (dp[i][w] !== dp[i - 1][w]) {
      selected.push(items[i - 1].category);
      w -= items[i - 1].cost;
    }
  }
